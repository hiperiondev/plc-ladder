...
void ut_char() {
    ...
    CU_ASSERT(result == (uint8_t) STATUS_ERR);
}

void ut_number() {
    int result = parse_il_extract_number(NULL);
    CU_ASSERT(result == STATUS_ERR);

    char *line = "lol lol";
    result = parse_il_extract_number(line);
    CU_ASSERT(result == STATUS_ERR);

    line = "lol52a";

    result = parse_il_extract_number(line + 5);
    CU_ASSERT(result == STATUS_ERR);

    result = parse_il_extract_number(line + 4);
    CU_ASSERT(result == 2);

    result = parse_il_extract_number(line + 3);
    CU_ASSERT(result == 52);
...
void ut_comments() {
    parse_il_read_line_trunk_comments(NULL);
    char line[MAXSTR];
    memset(line, 0, MAXSTR);
    sprintf(line, "%s", "lol");
    parse_il_read_line_trunk_comments(line);
...
void ut_whitespace() {
    ...
    parse_il_trunk_whitespace(line);
...
void ut_label() {
    parse_il_trunk_label(NULL, NULL, NULL);
    ...
    sprintf(line, "%s", "lol");
    parse_il_trunk_label(line, buf, result);
...
void ut_modifier() {
    ...
    uint8_t result = parse_il_read_modifier(NULL, NULL);
    CU_ASSERT(result == (uint8_t) STATUS_ERR);

    char *target = "";
    sprintf(line, "%s", "lol");
    result = parse_il_read_modifier(line, &target);
...
void ut_operator() {
    ...
    uint8_t result = parse_il_read_operator(NULL, NULL);
    CU_ASSERT(result == (uint8_t) STATUS_ERR);

    sprintf(line, "%s", "LT");
    result = parse_il_read_operator(line, NULL);
    CU_ASSERT(result == IL_LT);

    sprintf(line, "%s", "XOR lwieufql");
    result = parse_il_read_operator(line, line + 3);
...
void ut_arguments() {
    int result = parse_il_find_arguments(NULL, NULL, NULL, NULL);
    CU_ASSERT(result == STATUS_ERR);
    ...
    result = parse_il_find_arguments(line, &operand, &byte, &bit);
...
void ut_parse_real() {
    ...
    rung_t *rung_prg = parse_il_program("knuth.il", lines);
    result = (*rung_prg)->status;
...
void ut_parse() {
    ...
    rung_t *rung_prg = parse_il_program("gcd.il", lines);
    result = (*rung_prg)->status;
